...
)
{
	 ...
	 *
	 * Let `length1 := eval{""}` and `length2 := eval{"b"}`. `length2 - length1
	 * - power + 1` is the source _multiplicity_, or how many times each source
	 * is included in the word because of external variable products. `length1`
	 * is also the base length of the word group.
	 *
