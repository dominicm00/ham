 ...
 */
#ifndef HAM_DATA_TARGET_HPP
#define HAM_DATA_TARGET_HPP
...
class Target
{
  ...
  public:
	...
	inline const VariableDomain* Variables() const { return fVariables; }

	uint32_t Flags() const { return fFlags; }
	void SetFlags(uint32_t flags) { fFlags = flags; }
	void AddFlags(uint32_t flags) { fFlags |= flags; }
	bool IsBuildAlways() const { return (fFlags & BUILD_ALWAYS) != 0; }
	bool IsDontUpdate() const { return (fFlags & DONT_UPDATE) != 0; }
	inline bool IsIgnoreIfMissing() const;
	bool IsNotAFile() const { return (fFlags & NOT_A_FILE) != 0; }
	bool IsTemporary() const { return (fFlags & TEMPORARY) != 0; }
	inline bool DependsOnLeaves() const;

	const TargetSet& Dependencies() const { return fDependencies; }
	void AddDependency(Target* dependency) { fDependencies.Append(dependency); }
	inline void AddDependencies(const TargetSet& dependencies);

	const TargetSet& Includes() const { return fIncludes; }
	void AddInclude(Target* include) { fIncludes.Append(include); }
	...
	bool HasActionsCalls() const { return !fActionsCalls.empty(); }
	inline void AddActionsCall(RuleActionsCall* actionsCall);
