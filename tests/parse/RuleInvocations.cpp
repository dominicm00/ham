...
const auto parse = genericParse<RuleActionInvocation>;
...
TEST_CASE("Rule invocation, no argument", "[grammar]")
{
	...
	INFO(rule);
	REQUIRE_PARSE(rule, T<RuleActionInvocation>({T<Identifier>(rule)}));
}
...
TEST_CASE("Rule invocation, 1 argument", "[grammar]")
{
	REQUIRE_PARSE(
		"Rule a",
		T<RuleActionInvocation>({T<Identifier>("Rule"), T<List>({T<Leaf>("a")})}
		)
	);
	REQUIRE_PARSE(
		"Rule a b c",
...
TEST_CASE("Rule invocation, empty arguments", "[grammar]")
{
	...
	REQUIRE_PARSE(
		"Rule :",
		T<RuleActionInvocation>({T<Identifier>("Rule"), T<RuleSeparator>()})
	);
}
